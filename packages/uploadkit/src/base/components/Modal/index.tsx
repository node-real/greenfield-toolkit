  ...
  return (
    <Portal>
      <Transition in={isOpen} variant="fade">
        <Box className={cx('uk-modal', clsModal, className)} {...restProps}>
          <Box
            className={cx('uk-modal-overlay', clsModalOverlay)}
            onClick={() => closeOnOverlayClick && onClose()}
          />
          <Transition in={isOpen} variant={isMobileLayout ? 'modal-slide' : undefined}>
            <Box className={cx('uk-modal-content', clsModalContent, contentClassName)}>
              {children}
            </Box>
          </Transition>
        </Box>
